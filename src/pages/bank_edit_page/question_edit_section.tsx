  ...
  const createAiResponseHandler = () => {
    ...
    return (aiResponse: { status: string; data?: string }) => {
      ...
      if (aiResponse.status === "data") {
        ...
        const newQuestionTags = newQuestion.tags;
        newQuestion.tags = [];
        for (const tagName in newQuestionTags) {
          const tag = availableTags.find(tag => tag.name === tagName)
          if (tag) {
            newQuestion.tags.push(tag);
          }
          else if (tagName === "AI_GENERATED") {
            newQuestion.tags.push({ name: tagName, id: tagName, description: "This question is generated by AI", color: "#3b82f6" });
          }
          else {
            newQuestion.tags.push({ name: tagName, id: tagName, description: "", color: "#7c3aed" });
          }
        }
